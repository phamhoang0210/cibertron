...
class PriceFiltersFormBox extends React.Component {
  constructor(props) {
    super(props)
  ...
  getInitialValues() {
    ...
    return {
      created_at: getInitialValueForRangePicker({}, currentPriceParams, ['created_at.gte'], ['created_at.lt']),
      updated_at: getInitialValueForRangePicker({}, currentPriceParams, ['updated_at.gte'], ['updated_at.lt']),
      campaign_code: getInitialValue({}, currentPriceParams, ['campaign_code.in']),
      staff_id: getInitialValue({}, currentPriceParams, ['compconds', 'staff_id.in']),
      order_level_id: getInitialValue({}, currentPriceParams, ['compconds', 'order_level_id.in']),
      payment_method_ids: getInitialValue({}, currentPriceParams, ['payment_method_ids']),
    }
  }
  ...
  handleFilter(e) {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        let priceParams = getFilterParamsAndSyncUrl(indexState.get('priceFilters'), location, this.formatFormData(values))
  ...
  handleExport() {
    const {actions, indexState, location} = this.props
    let priceParams = getFilterParams(indexState.get('priceFilters'), location)
    const query = qs.stringify({...priceParams, ...getCredentials()}, { arrayFormat: 'brackets' })
          ...
          <Form onSubmit={this.handleFilter} >
            ...
            <Row>
              <Col span={24} style={{ textAlign: 'right' }}>
                ...
                <Button type="primary" htmlType="submit" loading={isFetchingPrices}>
...
export default Form.create()(injectIntl(PriceFiltersFormBox))